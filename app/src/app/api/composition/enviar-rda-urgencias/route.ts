...
export async function POST(request: Request) {
  try {
    ...
    console.log("[FHIR] Processing RDA Urgencias...");
    
    await logApiEvent({ event_id: crypto.randomUUID(), api_key: apiKey, endpoint: '/Composition/$enviar-rda-urgencias', timestamp: new Date().toISOString(), status_code: 200, duration_ms: 12 });
