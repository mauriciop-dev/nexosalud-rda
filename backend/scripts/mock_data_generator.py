...
def generate_mock_data():
    ...
    core_patients = [
        {"tipo_documento": "CC", "documento": "1018456789", "nombre_completo": "Evaristo Thomas", "fecha_nacimiento": "1975-05-12", "sexo": "M", "tenant_id": TENANT_ID},
        {"tipo_documento": "CC", "documento": "52345678", "nombre_completo": "Elena García", "fecha_nacimiento": "1988-11-20", "sexo": "F", "tenant_id": TENANT_ID},
        {"tipo_documento": "CC", "documento": "80123456", "nombre_completo": "Juan Pérez", "fecha_nacimiento": "1960-01-15", "sexo": "M", "tenant_id": TENANT_ID}
    ]

    # Inserción de pacientes core
    patient_ids = []
    for p in core_patients:
        res = supabase.table("patients").upsert(p, on_conflict="documento,tipo_documento").execute()
    ...
    for i in range(20):
        ...
        mock_fhir = {
            "resourceType": "Bundle",
            "type": "transaction",
            "entry": [{"resource": {"resourceType": "Patient", "name": [{"text": "Mock Name"}]}}]
        }
        
        codigo_vida = f"VIDA-{uuid.uuid4().hex[:8].upper()}" if estado == "Exitoso" else None
        ...
        res_rda = supabase.table("rda_records").insert(rda_record).execute()
        ...
        print(f"  [{i+1}/20] RDA {tipo} - Estado: {estado} (Paciente ID: {pid[:8]}...)")
